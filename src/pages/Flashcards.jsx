...
import { Layers, Plus, BookOpen, AlertCircle, RefreshCw, Check, ArrowLeft, ArrowRight, Eye, HelpCircle, Edit2, Trash2, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6">
      {!reviewMode ? (
        <>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-text-secondary text-sm">Ôn tập thông minh qua phương pháp Lặp lại ngắt quãng (Spaced Repetition).</p>
            </div>
            <button
              onClick={() => { setEditingDeck(null); setNewTitle(''); setNewDesc(''); setShowCreateModal(true); }}
              className="flex items-center space-x-2 bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-xl text-sm font-semibold cursor-pointer shadow-md transition duration-200"
            >
          ...
          {loading ? (
            ...
            <div className="border border-dashed border-border rounded-3xl p-12 text-center space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {decks.map(deck => (
                <div
                  ...
                  className="bg-surface border border-border rounded-3xl p-5 hover:border-primary/40 cursor-pointer shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between group h-44"
                >
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div>
            <h2 className="text-xl font-extrabold text-text-primary">{activeDeck?.title}</h2>
          ...
          {studyTab === 'study' ? (
            cards.length === 0 ? (
              <div className="border border-border rounded-3xl p-12 text-center bg-surface">
              ...
              <div className="space-y-8">
                ...
                >
                  <div className={`relative w-full h-full duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                    {/* Front Side */}
                    <div className="absolute inset-0 w-full h-full bg-surface border-2 border-border/80 rounded-3xl p-8 flex flex-col justify-between shadow-lg backface-hidden">
                    ...
                    {/* Back Side */}
                    <div className="absolute inset-0 w-full h-full bg-primary/5 dark:bg-primary/10 border-2 border-primary/30 rounded-3xl p-8 flex flex-col justify-between shadow-lg rotate-y-180 backface-hidden">
                      <div className="flex items-center space-x-2 text-primary">
