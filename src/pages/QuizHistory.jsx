    ...
    <div className="max-w-[1600px] w-full mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-border mb-6 gap-4">
        {!isAdmin ? (
          ...
          <div className="py-3">
            <h1 className="text-xl font-bold text-text-primary">Quản lý bài ôn tập (Admin)</h1>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto mb-2 sm:mb-0">
          ...
          {!loading && ((activeTab === 'history' && attempts.length > 0) || (activeTab === 'quizzes' && quizzes.length > 0)) && (
            <div className="relative w-full sm:w-64">
              ...
              <input
                type="text"
                placeholder={activeTab === 'history' ? "Tìm lịch sử..." : "Tìm Quiz..."}
                value={searchQuery}
        ...
        <div className="space-y-4">
          {attempts.length === 0 ? (
            ...
            <div className="bg-surface border border-border rounded-xl p-12 text-center text-text-secondary space-y-2">
              ...
              <p className="text-sm font-bold text-text-primary">Không tìm thấy lịch sử phù hợp</p>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1 text-xs text-text-secondary">
                        <p className="line-clamp-1">
                          Điểm số: <span className="font-bold text-text-primary">{att.score}/{att.quiz?.questions?.length || 5} câu</span> ({scorePercentage}%)
                        </p>
                        <p className="line-clamp-1">
                          Thời gian làm: {formatDuration(att.time_spent)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-border/60 pt-3 flex items-center justify-between text-xs text-text-secondary">
                      ...
                      <span className="ml-auto font-semibold flex items-center space-x-1 shrink-0 text-primary group-hover:underline">
                        ...
                        <span>{isCompleted ? 'Xem kết quả' : 'Làm tiếp'}</span>
                      </span>
                    </div>
                  </div>
                );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quizzes.length === 0 ? (
            ...
            <div className="col-span-full bg-surface border border-border rounded-xl p-12 text-center text-text-secondary space-y-2">
              <Search className="w-12 h-12 text-text-secondary/30 mx-auto" />
              <p className="text-sm font-bold text-text-primary">Không tìm thấy Quiz phù hợp</p>
              <p className="text-xs text-text-secondary">Vui lòng thử lại với từ khóa khác.</p>
            </div>
          ) : (
            <>
              {paginatedQuizzes.map((quiz) => (
                ...
                >
                  <div className="space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5 flex-1 min-w-0">
                        <h3 className="font-bold text-text-primary text-base group-hover:text-primary transition-colors line-clamp-1 flex-1" title={quiz.title?.replace(/^Quiz:\s*/i, '').replace(/^Flashcard:\s*/i, '')}>
                          {quiz.title?.replace(/^Quiz:\s*/i, '').replace(/^Flashcard:\s*/i, '')}
                        </h3>
                      </div>
                  ...
                  <div className="border-t border-border/60 pt-3 flex items-center justify-between text-xs text-text-secondary">
                    ...
                    <span className="ml-auto font-semibold flex items-center space-x-1 shrink-0 text-primary group-hover:underline">
                      ...
                      <span>{isAdmin ? 'Xem kết quả' : 'Làm bài'}</span>
                    </span>
                  </div>
                </div>
              ))}
